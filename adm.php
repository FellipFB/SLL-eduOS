...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($id <= 0 || empty($title) || empty($author) || empty($publisher)) {
            ...
            if (!empty($isbnClean) && strlen($isbnClean) !== 10 && strlen($isbnClean) !== 13) {
                ...
                try {
                    ...
                    header("Location: adm.php?tab=acervo&status=edited");
                    exit;
                } catch (PDOException $e) {
    ...
    if ($action === 'add_admin') {
        ...
        if (empty($name)) {
            $error_message = 'O nome completo é obrigatório.';
        } elseif (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $error_message = 'Por favor, informe um e-mail válido.';
        } elseif (strlen($password) < 6) {
            $error_message = 'A senha deve conter pelo menos 6 caracteres.';
        } else {
            try {
                $stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM `adm_usuarios` WHERE LCASE(`email`) = LCASE(?)");
                $stmtCheck->execute([$email]);
                if ($stmtCheck->fetchColumn() > 0) {
                    $error_message = 'Já existe uma conta administrativa cadastrada com este e-mail.';
                } else {
                    $passwordHash = password_hash($password, PASSWORD_DEFAULT);
                    $stmtInsertAdm = $pdo->prepare("INSERT INTO `adm_usuarios` (`name`, `email`, `password`) VALUES (?, ?, ?)");
                    $stmtInsertAdm->execute([$name, $email, $passwordHash]);
                    $success_message = 'Novo administrador cadastrado com sucesso!';
                }
            } catch (PDOException $e) {
                $error_message = 'Erro ao cadastrar administrador: ' . $e->getMessage();
            }
        }
    }
    ...
    if ($action === 'delete_admin') {
        ...
        if ($id > 0) {
            try {
                // Obter email para verificar se é o mestre admin
                $stmtCheck = $pdo->prepare("SELECT `email` FROM `adm_usuarios` WHERE `id` = ?");
                $stmtCheck->execute([$id]);
                $email = $stmtCheck->fetchColumn();

                if ($email && strtolower($email) === 'admin@tecavirtual.com') {
                    $error_message = 'O administrador master padrão é protegido e não pode ser removido.';
                } else {
                    $stmtDel = $pdo->prepare("DELETE FROM `adm_usuarios` WHERE `id` = ?");
                    $stmtDel->execute([$id]);
                    $success_message = 'Administrador removido com sucesso!';
                }
            } catch (PDOException $e) {
                $error_message = 'Erro ao remover administrador: ' . $e->getMessage();
            }
        }
    }
}
...
try {
    ...
    // Listagem total de administradores
    $administradoresList = $pdo->query("SELECT * FROM `adm_usuarios` ORDER BY `name` ASC")->fetchAll();
    $totalAdminsCount = count($administradoresList);
...
<body class="selection:bg-brand-primary/10">
    ...
    <div class="w-full max-w-7xl mx-auto px-4 py-8 space-y-8" id="main-layout">
        ...
        <header id="dashboard-navbar" class="bg-[#fdfdfc] px-8 py-5 rounded-2xl border border-brand-border editorial-shadow flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 justify-between md:justify-end" id="navbar-user-area">
                <div class="text-right hidden sm:block">
                    ...
                    <p class="text-xs text-brand-light italic"><?= htmlspecialchars($_SESSION['user_name'] ?? $_SESSION['user_email'] ?? 'Administrador') ?></p>
                </div>
                <a
                    href="logout.php"
                    id="btn-logout"
        ...
        <nav class="flex border-b border-brand-border/40 pb-0.5 gap-6" id="dashboard-tab-navigation">
            ...
            <a 
                href="?tab=admins" 
                class="pb-4 px-1 border-b-2 text-xs font-bold uppercase tracking-widest transition-all gap-2 flex items-center <?= $activeTab === 'admins' ? 'border-brand-primary text-brand-primary' : 'border-transparent text-brand-light hover:text-brand-dark' ?>"
            >
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="bg-[#fdfdfc] p-6 rounded-2xl border border-brand-border editorial-shadow flex flex-col relative">
                        ...
                        <div class="border-b border-brand-border/40 pb-4 mb-5">
                            ...
                            <?php if ($editingBook): ?>
                                <a href="adm.php?tab=acervo" class="text-[10px] text-brand-secondary hover:underline tracking-tight mt-1 inline-block">← Cancelar Edição / Novo Cadastro</a>
                            <?php endif; ?>
                        </div>
        ...
        <?php if ($activeTab === 'admins'): ?>
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start" id="view-admins">
                
                <!-- 1. LEFT COLUMN: Adicionar Administrador Form -->
                <div class="lg:col-span-15 space-y-6 lg:col-span-5">
                    <div class="bg-white p-6 rounded-2xl border border-brand-border editorial-shadow flex flex-col">
                        ...
                        <div class="border-b border-brand-border/40 pb-4 mb-5">
                            ...
                            <h3 class="text-xl font-normal text-brand-dark font-serif italic">Novo Administrador</h3>
                        </div>
                        ...
                        <form method="POST" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Nome Completo <span class="text-red-500">*</span></label>
                                <input 
                                    type="text" 
                                    name="name" 
                                    required 
                            ...
                            <div>
                                <label class="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">E-mail de Login <span class="text-red-500">*</span></label>
                                <input 
                                    type="email" 
                                    name="email" 
                                    required 
                                    placeholder="Ex: ruan@tecavirtual.com"
                                    class="w-full bg-[#f8fafc] border border-brand-border/60 p-2.5 text-xs rounded-xl text-brand-dark placeholder-[#8ea2c0] outline-none focus:border-brand-primary"
                                />
                            </div>

                            <div>
                                <label class="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Senha de Acesso <span class="text-red-500">*</span></label>
                                <input 
                                    type="password" 
                                    name="password" 
                                    required 
                            ...
                            <button 
                                type="submit" 
                                class="w-full py-2.5 bg-brand-primary hover:bg-[#153468] text-white text-xs font-bold rounded-full uppercase tracking-wider transition-colors shadow-sm cursor-pointer"
                            >
                ...
                <!-- 2. RIGHT COLUMN: Listagem de Administradores Cadastrados no Banco -->
                <div class="lg:col-span-7 space-y-6">
                    <div class="bg-white p-6 rounded-2xl border border-brand-border editorial-shadow">
                        
                        <div class="mb-5 border-b border-brand-border/40 pb-4">
                            <h3 class="text-xl font-normal text-brand-dark font-serif italic">Administradores Ativos no Banco</h3>
                        </div>

                        <div class="space-y-3">
                            <?php foreach ($administradoresList as $adm): ?>
                                <div class="p-4 rounded-xl border border-brand-border/50 bg-[#fdfdfc] flex items-center justify-between text-xs hover:border-brand-border transition-all">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2">
                                            <span class="font-bold text-brand-dark text-[13px]"><?= htmlspecialchars($adm['name']) ?></span>
                                            <?php if (strtolower($adm['email']) === 'admin@tecavirtual.com'): ?>
                                                <span class="text-[9px] bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded font-bold uppercase tracking-wider">Master</span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="text-[11px] text-[#5a6f95] font-mono"><?= htmlspecialchars($adm['email']) ?></div>
                                        <div class="text-[9px] font-mono text-slate-400 capitalize tracking-wider">Criado em: <?= date('d/m/Y H:i', strtotime($adm['created_at'])) ?></div>
                                    </div>
                                    
                                    <?php if (strtolower($adm['email']) !== 'admin@tecavirtual.com'): ?>
                                        <form method="POST" onsubmit="return confirm('Tem certeza absoluta que deseja revogar o acesso administrativo de <?= htmlspecialchars($adm['name']) ?>?');">
                                            <input type="hidden" name="action" value="delete_admin">
                                            <input type="hidden" name="id" value="<?= $adm['id'] ?>">
                                            <button 
                                                type="submit" 
                                                class="p-1 px-3 border border-brand-border hover:bg-red-50 hover:border-red-200 text-red-600 rounded-lg text-[10px] font-bold tracking-wide uppercase transition duration-150 cursor-pointer"
                                    ...
                                    <?php else: ?>
                                        <span class="text-[10px] text-slate-400 italic">Inalterável</span>
                                    <?php endif; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
