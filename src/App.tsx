...
export default function App() {
  const [viewMode, setViewMode] = useState<'reader' | 'admin'>(() => {
    const stored = localStorage.getItem('tecavirtual_viewmode');
    return (stored as 'reader' | 'admin') || 'reader';
  });

  useEffect(() => {
    localStorage.setItem('tecavirtual_viewmode', viewMode);
  }, [viewMode]);
  ...
  const [admins, setAdmins] = useState<AdminAccount[]>(() => {
    const stored = localStorage.getItem('tecavirtual_admins');
    if (stored) {
      try {
    ...
    return [
      { name: 'Administrador Geral', email: 'admin@tecavirtual.com', password: 'admin123' }
    ];
  });

  // Sync admins with LocalStorage when state changes
  useEffect(() => {
    localStorage.setItem('tecavirtual_admins', JSON.stringify(admins));
  }, [admins]);
  ...
  const handleRemoveAdmin = (email: string) => {
    // Protect the master admin email from configuration removal
    if (email.toLowerCase() === 'admin@tecavirtual.com') return;
    setAdmins((prev) => prev.filter((adm) => adm.email.toLowerCase() !== email.toLowerCase()));
  };
  ...
  const handleUpdateBookStatus = (bookId: string, status: Book['status']) => {
    setBooks((prev) =>
      prev.map((b) => (b.id === bookId ? { ...b, status: status } : b))
    );
  };
  ...
  return (
    <div id="app-root-container" className="min-h-screen bg-transparent py-4 sm:py-8 font-sans">
      <AnimatePresence mode="wait">
        {viewMode === 'reader' ? (
          <motion.div
            key="reader-storefront"
            id="reader-storefront-wrap"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <Login
              ...
              onBackToReader={() => setViewMode('reader')}
            />
          </motion.div>
        ) : (
          <motion.div
